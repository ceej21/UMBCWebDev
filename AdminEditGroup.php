...
<html lang="en">
  ...
  <body>
      ...
      <div id="form">
        <div class="top">
          ...
          <?php
			...
			//gets all group appointments
            $sql = "SELECT * FROM `Proj2Appointments` WHERE `AdvisorID` = '0' ORDER BY `Time`";
            $rs = $COMMON->executeQuery($sql, "Advising Appointments");
            $row = mysql_fetch_array($rs, MYSQL_NUM); 
			//first item in row, if it exists print a table of radio buttons representing for each appointment
            if($row){
	...
	echo("<tr><td width='320px'>Time</td><td>Majors</td><td>Seats Enrolled</td><td>Total Seats</td></tr>\n");
              ...
              //determine if there are specified major and print it out
              if($row[3]){
                echo("<td>".$row[3]."</td>"); 
              }
              else{
			...
			//does the same as above but with the rest of data in row
              echo("</td></tr>\n");
              while ($row = mysql_fetch_array($rs, MYSQL_NUM)) {
                echo("<tr><td><label for='$row[0]'><input type=\"radio\" id='$row[0]' name=\"GroupApp\" 
                  required value=\"row[]=$row[1]&row[]=$row[3]&row[]=$row[5]&row[]=$row[6]\">");
                echo(date('l, F d, Y g:i A', strtotime($row[1])). "</label></td>");
                //checks if there is a specified major
                if($row[3]){
                  echo("<td>".$row[3]."</td>"); 
                }
                else{
		...
		echo("</table>");
            ...
            //otherwise the admin cannot edit anything
            else{
